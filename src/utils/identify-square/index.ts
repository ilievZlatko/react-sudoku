...
/**
 * A function that identifies and returns the current square of a given sudoku grid
 * @param input Object with 9x9 sudoku grid, row index and col index
 */
function identifuWorkingSquare({ col, grid, row }: IInput): SQUARE {
  const square = []
  if (row < 3) {
    if (col < 3) {
      for (let x = 0; x < 3; x++) {
...
export default identifuWorkingSquare
